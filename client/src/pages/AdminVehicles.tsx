...
import { vehicleBrands as catalogBrands, vehicleCatalog } from "@/config/vehicleCatalog";
import { displayPrice } from "@/lib/vehicleDetail";
import { trpc } from "@/lib/trpc";
import { Check, ChevronDown, ChevronUp, Download, FileSpreadsheet, ImagePlus, Loader2, Save, Tags, Trash2, UploadCloud } from "lucide-react";
...
type BrandCard = { id: number; brandName: string; displayName: string; logoUrl: string | null; logoKey: string | null; sortOrder: number; isVisible: boolean };
...
function AdminVehiclesContent() {
  ...
  const [tab, setTab] = useState<StudioTab>("vehicle");
  ...
  const brandCards = useMemo<BrandCard[]>(() => {
    ...
    return brandNames.map((brandName, index) => {
      ...
      return saved ? { id: saved.id, brandName: saved.brandName, displayName: saved.displayName, logoUrl: saved.logoUrl, logoKey: saved.logoKey, sortOrder: saved.sortOrder, isVisible: saved.isVisible } : { id: -(index + 1), brandName, displayName: brandName, logoUrl: null, logoKey: null, sortOrder: index, isVisible: true };
    });
  ...
  const onBrandLogo = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const uploaded = await uploadLogo.mutateAsync({ fileName: file.name, contentType: file.type as "image/jpeg" | "image/png" | "image/webp", base64: await fileToBase64(file) });
  ...
  return <main className="admin-vehicle-studio">
    <header className="admin-studio-header"><div><p className="eyebrow">ZAVERRE / MANAGEMENT</p><h1>Vehicle studio</h1></div><button className="button button-quiet" onClick={() => navigate("/")}>VIEW PUBLIC SITE</button></header>
    <nav className="studio-tab-nav" aria-label="Vehicle Studio sections"><button className={tab === "vehicle" ? "active" : ""} onClick={() => setTab("vehicle")}>VEHICLE &amp; GALLERY</button><button className={tab === "import" ? "active" : ""} onClick={() => setTab("import")}><FileSpreadsheet size={14} /> SHEET IMPORT</button><button className={tab === "brands" ? "active" : ""} onClick={() => setTab("brands")}><Tags size={14} /> BRAND MANAGER</button><button className={tab === "prices" ? "active" : ""} onClick={() => setTab("prices")}>BULK PRICE EDITOR · {vehicleCatalog.length}</button></nav>
    ...
    {tab === "brands" && <section className="studio-panel"><div className="studio-section-heading"><div><p className="eyebrow">PUBLIC FILTER IDENTITIES</p><h2>Brand manager</h2><p>Select any current catalogue brand to add or replace its logo, or add a new brand. Each visible identity is available for vehicle assignment and is created automatically in the public fleet filter when it has listed vehicles.</p></div></div><div className="brand-manager-layout"><form className="brand-manager-form" onSubmit={(event) => { event.preventDefault(); onSaveBrand(); }}><div className="brand-manager-form-head"><h3>{brandForm.brandName ? "Edit brand" : "New brand"}</h3><button className="gallery-action" type="button" onClick={() => setBrandForm({ brandName: "", displayName: "", logoUrl: "", logoKey: "", sortOrder: "0", isVisible: true })}>CLEAR</button></div><div className="admin-field-grid"><label>BRAND NAME<input value={brandForm.brandName} placeholder="e.g. Bugatti" onChange={(event) => setBrandForm((current) => ({ ...current, brandName: event.target.value }))} /></label><label>PUBLIC DISPLAY NAME<input value={brandForm.displayName} placeholder="e.g. Bugatti" onChange={(event) => setBrandForm((current) => ({ ...current, displayName: event.target.value }))} /></label><label>FILTER ORDER<input value={brandForm.sortOrder} inputMode="numeric" onChange={(event) => setBrandForm((current) => ({ ...current, sortOrder: event.target.value }))} /></label><label className="admin-toggle"><input checked={brandForm.isVisible} type="checkbox" onChange={(event) => setBrandForm((current) => ({ ...current, isVisible: event.target.checked }))} />Visible in filter</label></div><div className="brand-logo-upload"><div className="brand-logo-preview">{brandForm.logoUrl ? <img src={brandForm.logoUrl} alt="Brand logo preview" /> : <span>LOGO<br />PREVIEW</span>}</div><label className="admin-upload"><ImagePlus size={16} />{uploadLogo.isPending ? "UPLOADING…" : "UPLOAD LOGO"}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={onBrandLogo} disabled={uploadLogo.isPending} /></label></div><button type="submit" className="button button-gold" style={{ marginTop: 18 }} disabled={saveBrand.isPending || !brandForm.brandName.trim() || !brandForm.displayName.trim()}>{saveBrand.isPending ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />}SAVE BRAND</button>{saveBrand.isSuccess && <p className="admin-feedback"><Check size={15} />Brand saved. Assign it to vehicles in the Vehicle tab.</p>}</form><div className="brand-list">{brands.isLoading && <p>Loading brands…</p>}{brandCards.map((brand) => <article key={brand.brandName} className="brand-card"><div className="brand-card-top"><span className="brand-card-logo"><BrandMark brandName={brand.brandName} logoUrl={brand.logoUrl} /></span><div><strong>{brand.displayName}</strong><small>{brand.brandName}</small></div></div><small>{brand.logoUrl ? "CUSTOM LOGO READY" : "CATALOGUE IDENTITY"} · ORDER {brand.sortOrder}</small><button className="brand-card-action" onClick={() => editBrand(brand)}>EDIT / ADD LOGO</button></article>)}</div></div></section>}
