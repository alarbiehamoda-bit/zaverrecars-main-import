...
function AdminVehiclesContent() {
  ...
  const onBrandLogo = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!["image/jpeg", "image/png", "image/webp", "image/svg+xml"].includes(file.type)) {
      setBrandMessage({ type: "error", text: "Use a JPEG, PNG, WEBP or SVG logo file." });
      return;
    }
    if (file.size > 8 * 1024 * 1024) {
      setBrandMessage({ type: "error", text: "The logo must be 8 MB or smaller." });
      return;
    }
    try {
  ...
  const onSaveBrand = () => {
    if (!brandForm.brandName.trim() || !brandForm.displayName.trim()) {
      setBrandMessage({ type: "error", text: "Brand name and public display name are required before saving." });
      return;
    }
    saveBrand.mutate({ brandName: brandForm.brandName.trim(), displayName: brandForm.displayName.trim(), logoUrl: brandForm.logoUrl || null, logoKey: brandForm.logoKey || null, sortOrder: Number(brandForm.sortOrder) || 0, isVisible: brandForm.isVisible });
  };
  const editBrand = (brand: BrandCard) => {
    setBrandForm({ brandName: brand.brandName, displayName: brand.displayName, logoUrl: brand.logoUrl || "", logoKey: brand.logoKey || "", sortOrder: String(brand.sortOrder), isVisible: brand.isVisible });
    setBrandMessage({ type: "success", text: `Editing ${brand.displayName}. Upload a replacement logo if needed, then choose Save Brand.` });
  };
  const resetBrandForm = () => {
    setBrandForm({ brandName: "", displayName: "", logoUrl: "", logoKey: "", sortOrder: "0", isVisible: true });
