...
import { Activity, ArrowUpRight, BookOpenText, CarFront, ClipboardList, Loader2, Tags, UsersRound } from "lucide-react";
...
function AdminOperationsPage() {
  ...
  return <main className="operations-page"><header className="operations-heading"><div><p className="eyebrow">ZAVERRE / OPERATIONS</p><h1>Operations cockpit</h1><p>One operational view for sales leads, fleet presentation, brands and website content.</p></div><button className="operations-refresh" onClick={() => overview.refetch()}>Refresh data</button></header><section className="operations-kpis">{metrics.map((metric) => <article key={metric.label} className={`operations-kpi ${metric.tone}`}><span>{metric.label}</span><strong>{metric.value}</strong><small>{metric.detail}</small></article>)}</section><section className="operations-grid"><div className="operations-workspace"><div className="operations-section-heading"><div><p className="eyebrow">WORKSPACES</p><h2>Run the business</h2></div><UsersRound size={18} /></div><div className="operations-workspace-grid">{workspaces.map((workspace) => <button key={workspace.path} className={`operations-workspace-card ${workspace.tone}`} onClick={() => navigate(workspace.path)}><workspace.icon size={22} /><div><h3>{workspace.title}</h3><p>{workspace.detail}</p></div><ArrowUpRight size={17} /></button>)}</div></div><aside className="operations-activity"><div className="operations-section-heading"><div><p className="eyebrow">ACTIVITY LOG</p><h2>Recent operational changes</h2></div><Activity size={18} /></div>{data.activity.length ? <ol>{data.activity.map((entry) => <li key={entry.id}><span>{entry.action.replaceAll(".", " · ")}</span><small>{entry.subjectType}{entry.subjectKey ? ` · ${entry.subjectKey}` : ""} · {new Date(entry.createdAt).toLocaleString()}</small></li>)}</ol> : <div className="operations-empty">Actions such as updates to booking status, vehicle content, prices and brands will appear here.</div>}</aside></section><section className="operations-queue"><div className="operations-section-heading"><div><p className="eyebrow">BOOKING QUEUE</p><h2>Latest requests</h2></div><button onClick={() => navigate("/admin/bookings")}>Open inbox <ArrowUpRight size={14} /></button></div>{data.recentBookings.length ? <div className="operations-queue-list">{data.recentBookings.map((booking) => <button key={booking.id} onClick={() => navigate("/admin/bookings")}><span className={`operations-status ${booking.status}`}>{booking.status}</span><strong>{booking.fullName}</strong><span>{booking.vehicleKey}</span><time>{new Date(booking.createdAt).toLocaleDateString()}</time></button>)}</div> : <div className="operations-empty">No booking requests have been recorded yet.</div>}</section></main>;
}
