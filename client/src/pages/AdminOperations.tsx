...
import { Activity, ArrowUpRight, BookOpenText, CarFront, CircleDollarSign, ClipboardList, FileSpreadsheet, Loader2, Sparkles, Tags, UsersRound } from "lucide-react";
...
function AdminOperationsPage() {
  ...
  const publicCms = trpc.cms.public.useQuery();
  const publicBrands = trpc.brand.publicPresentationList.useQuery();
  ...
  const delivery = [
    { title: "Brand assets & uploads", detail: publicBrands.isFetching ? "Refreshing public brand source…" : `${publicBrands.data?.length ?? 0} brand presentations are available to the public site.`, path: "/admin/brands", icon: Tags, state: publicBrands.isError ? "attention" : "connected" },
    { title: "Homepage content & design", detail: publicCms.isFetching ? "Refreshing public content source…" : publicCms.isError ? "Public content source needs attention." : publicCms.data?.settings.length ? `${publicCms.data.settings.length} public settings are connected to the homepage.` : "No public settings are published yet. Open Content studio and import current content once.", path: "/admin/content", icon: BookOpenText, state: publicCms.isError || !publicCms.data?.settings.length ? "attention" : "connected" },
    { title: "AI review workflow", detail: "Creates review proposals and opens the matching editor; it never writes public changes automatically.", path: "/admin/vehicles", icon: Sparkles, state: "review" },
  ] as const;
  return <main className="operations-page"><header className="operations-heading"><div><p className="eyebrow">ZAVERRE / OPERATIONS</p><h1>Operations cockpit</h1><p>One operational view for sales leads, fleet presentation, brands and website content.</p><p className="operations-review-note">Review-led workflow — opening a workspace never writes data. Every live change stays behind its dedicated save control.</p></div><button type="button" className="operations-refresh" onClick={() => { void overview.refetch(); void publicCms.refetch(); void publicBrands.refetch(); }}>Refresh data</button></header><section className="operations-kpis">{metrics.map((metric) => <article key={metric.label} className={`operations-kpi ${metric.tone}`}><span>{metric.label}</span><strong>{metric.value}</strong><small>{metric.detail}</small></article>)}</section><section className="operations-public-delivery" aria-label="Public delivery connection"><div className="operations-section-heading"><div><p className="eyebrow">PUBLIC DELIVERY</p><h2>Administration is connected to these public sources</h2></div><Activity size={18} /></div><div className="operations-delivery-grid">{delivery.map((item) => <button type="button" key={item.title} className={`operations-delivery-card ${item.state}`} onClick={() => navigate(item.path)}><item.icon size={19} /><div><strong>{item.title}</strong><span>{item.detail}</span></div><ArrowUpRight size={16} /></button>)}</div></section><section className="operations-grid"><div className="operations-workspace"><div className="operations-section-heading"><div><p className="eyebrow">WORKSPACES</p><h2>Run the business</h2></div><UsersRound size={18} /></div><div className="operations-workspace-grid">{workspaces.map((workspace) => <button type="button" key={workspace.path} className={`operations-workspace-card ${workspace.tone}`} onClick={() => navigate(workspace.path)}><workspace.icon size={22} /><div><h3>{workspace.title}</h3><p>{workspace.detail}</p></div><ArrowUpRight size={17} /></button>)}</div></div><aside className="operations-activity"><div className="operations-section-heading"><div><p className="eyebrow">ACTIVITY LOG</p><h2>Recent operational changes</h2></div><Activity size={18} /></div>{data.activity.length ? <ol>{data.activity.map((entry) => <li key={entry.id}><span>{entry.action.replaceAll(".", " · ")}</span><small>{entry.subjectType}{entry.subjectKey ? ` · ${entry.subjectKey}` : ""} · {new Date(entry.createdAt).toLocaleString()}</small></li>)}</ol> : <div className="operations-empty">Actions such as updates to booking status, vehicle content, prices and brands will appear here.</div>}</aside></section><section className="operations-queue"><div className="operations-section-heading"><div><p className="eyebrow">BOOKING QUEUE</p><h2>Latest requests</h2></div><button type="button" onClick={() => navigate("/admin/bookings")}>Open inbox <ArrowUpRight size={14} /></button></div>{data.recentBookings.length ? <div className="operations-queue-list">{data.recentBookings.map((booking) => <button type="button" key={booking.id} onClick={() => navigate("/admin/bookings")}><span className={`operations-status ${booking.status}`}>{booking.status}</span><strong>{booking.fullName}</strong><span>{booking.vehicleKey}</span><time>{new Date(booking.createdAt).toLocaleDateString()}</time></button>)}</div> : <div className="operations-empty">No booking requests have been recorded yet.</div>}</section></main>;
}
