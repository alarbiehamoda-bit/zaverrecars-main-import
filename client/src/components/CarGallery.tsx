...
export function CarGallery({ vehicleName, images, imageFit = "contain" }: { vehicleName: string; images: CarGalleryImage[]; imageFit?: "contain" | "cover" | "fill" }) {
  ...
  return <>
    ...
    <Dialog open={lightboxOpen} onOpenChange={setLightboxOpen}><DialogContent className="detail-lightbox"><DialogTitle className="sr-only">{vehicleName} image gallery</DialogTitle><button className="detail-lightbox-close" onClick={() => setLightboxOpen(false)}><X /></button><img src={gallery[activeImage]?.src} alt={gallery[activeImage]?.alt} style={{ objectFit: imageFit }} /></DialogContent></Dialog>
  </>;
}
